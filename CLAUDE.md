...
- **Claude 寫完文章一律開成 GitHub issue（`gh issue create`），不要直接寫進 `src/content/blog/` 也不要直接 commit 文章檔。** 檔案由 `issue-to-post.yml` 產生，不是由 Claude 寫。開完 issue 把 issue URL 給我就好。
- **開 issue 時預設貼 `draft` label**（`gh issue create --label draft`），**絕對不要貼 `published`**——`published` 是我自己確認完才貼的，貼下去就直接發佈兼部署了。
